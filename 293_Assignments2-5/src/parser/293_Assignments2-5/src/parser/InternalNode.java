...
public final class InternalNode implements Node {
	...
	public static class Builder {
		...
		public Builder simplify() {
			...
			children = simplifyGrandchild(children);
		...
		// helper method (reduces complexity), checks if there is a single internal node
		...
		// Simplifies the children list by replacing internal nodes with their children
		// starting with operators
		private List<Node> simplifyOperators(List<Node> children) {
			List<Node> newChildren = new ArrayList<Node>();
			for (int i = 0; i < children.size() - 1; i++) {
				Node tempNode = children.get(i);
				if (tempNode.isStartedByOperator() && (i == 0 || !children.get(i - 1).isOperator())) {
		...
		// Simplifies the children list by replacing internal nodes with their children
		// that are single leaves
		private List<Node> simplifyGrandchild(List<Node> children) {
			List<Node> newChildren = new ArrayList<Node>();
			for (int i = 0; i < children.size() - 1; i++) {
				Node tempNode = children.get(i);
